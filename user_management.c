...
#include <string.h>
#include <unistd.h>
// #include <termios.h> // Used for password masking, currently commented
...
int main() {
    ...
    while(1) {
        ...
        printf("\nSelect an option: ");
        scanf("%d", &option);
        getchar(); // ek character ko input se utha lega 
                   // here it is used to consume extra enter
...
void register_user() {
    ...
    // printf("\nEnter username: ");
    // fgets(users[new_index].username, CREDENTIAL_LENGTH, stdin); // fgets(input, length, stdin)
...
void input_credentials(char *username,char* password){
    printf("\nEnter username: ");
    fgets(username, CREDENTIAL_LENGTH, stdin); // fgets(input, length, stdin)
